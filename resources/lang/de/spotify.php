<?php

return [

    'statistic' => 'Statistik',
    'minutes' => [
        'short' => 'min',
        'singular' => 'Minute',
        'plural' => 'Minuten',
        'heared' => 'Minuten gehört'
    ],
    'title' => [
        'favourite_year' => 'Dein Lieblingsjahr',
        'favourite_bpm' => 'Deine Lieblingsgeschwindigkeit',
        'count_tracks' => 'Anzahl gehörter Lieder',
        'avg_session_length' => 'hörst du im Schnitt am Stück',
        'last_heared' => 'Zuletzt gehört',
        'top_tracks' => 'Deine Top Tracks',
        'top_artists' => 'Deine Top Künstler',
        'heared_minutes_by_week' => 'Gehörte Minuten nach Woche',
        'heared_minutes_by_weekday' => 'Gehörte Minuten nach Wochentag',
        'heared_minutes_by_daytime' => 'Gehörte Minuten nach Tageszeit',
        'lost_tracks' => 'Verschollene Tracks',
    ],
    'total' => 'gesamt',
    'last_days' => 'letzte :days Tage',
    'last_hours' => 'letzte :hours Stunden',
    'time_suffix' => 'Uhr',
    'rank' => 'Platz',
    'artist' => 'Künstler',
    'heared_minutes' => 'Hörzeit',
    'find_tracks_again' => 'Entdecke alte Lieder wieder!',
    'create_playlist' => 'Playlist erstellen',
    'show_playlist' => 'Playlist anzeigen',
    'lost_tracks' => [
        'your_lost_tracks' => 'Deine aktuell verschollenen Tracks',
        'no_tracks' => 'Du hast aktuelle keine verschollenen Tracks. Passe den Filter an oder schaue in einigen Tagen nochmal vorbei.',
        'max_tracks' => 'Maximale Lieder in der Playlist (max. :max)',
        'daycount_lost_tracks' => 'Anzahl an Tagen, bis ein Lied als "alt" gilt',
        'minutes_until_liked' => 'Gehörte Minuten, ab dem ein Lied als "Mag ich" eingestuft wird',
        'description' => 'KStats erstellt dir eine Playlist mit Tracks, die du länger nicht mehr gehört hast.'
    ],


];
